...
class PerplexityLLM(BaseLLM):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_endpoint = API_ENDPOINTS["perplexity"]
        self.api_key = os.getenv("PERPLEXITY_API_KEY")
        if not self.api_key:
            raise LLMValidationError("Perplexity API key not found in environment variables")
    ...
    async def format_prompt(self, prompt: str, **kwargs) -> Dict[str, Any]:
        """Format prompt for Perplexity API"""
        messages = []
